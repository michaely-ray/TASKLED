...
esp_err_t esp_modem_start_ppp(modem_dte_t *dte)
{
    ...
    MODEM_CHECK(dce, "DTE has not yet bind with DCE", err);
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
    int8_t operIndex = espModemFindOperator(dce->oper);
    if (operIndex == -1)
    {
        ESP_LOGE(MODEM_TAG, "Unknown operator :(");
        return ESP_FAIL;
    }
    /* Set PDP Context */
    MODEM_CHECK(dce->define_pdp_context(dce, 1, "IP", operators[operIndex].apn) == ESP_OK, "set MODEM APN failed", err);
    /* Enter PPP mode */
...
int8_t espModemFindOperator(char *operCode)
{
    // uint32_t operatorAsNumber;
    // sscanf(operCode, "\"%d\"", &operatorAsNumber);
    // operatorAsNumber -= 72400;

    if (strcmp(operCode, "\"NeXtel\"") == 0)
    {
        return nextel;
    }
    else if (strcmp(operCode, "\"TIM BRASIL\"") == 0)
    {

        return tim;
    }
    else if (strcmp(operCode, "\"Claro\"") == 0)
    {
        return claro;
    }
    else if (strcmp(operCode, "\"NeXtel\"") == 0)
    {
        return sercomtel;
    }
    else if (strcmp(operCode, "\"72411\"") == 0)
    {
        return vivo;
    }
    else if (strcmp(operCode, "\"TNL PCS S.A.\"") == 0)
    {
        return oi;
    }
    else if (strcmp(operCode, "\"NeXtel\"") == 0)
